  ...
  return (
    <>
      ...
      <header className="sticky top-0 z-50 w-full border-b bg-background/95 backdrop-blur supports-[backdrop-filter]:bg-background/60 shadow-sm">
        ...
        <nav className="hidden lg:block border-t bg-white relative z-[55] overflow-visible">
          <div className="container mx-auto px-4 overflow-visible">
            <ul className="flex justify-start xl:justify-center items-center h-14 space-x-3 xl:space-x-4 overflow-visible whitespace-nowrap">
              {navLinks.map((link) => (
                <li key={link.name} className="group relative h-full flex items-center">
                  ...
                  {link.subLinks && (
                    <div className="absolute top-full left-1/2 -translate-x-1/2 pt-2 opacity-0 invisible group-hover:opacity-100 group-hover:visible transition-all duration-300 z-[100] pointer-events-none group-hover:pointer-events-auto">
                       <div className="bg-background shadow-2xl rounded-2xl border p-10 w-[95vw] max-w-7xl grid grid-cols-5 gap-10 ring-1 ring-black/5 animate-in fade-in slide-in-from-top-2 duration-300">
                        {link.subLinks.map((subLink) => (
